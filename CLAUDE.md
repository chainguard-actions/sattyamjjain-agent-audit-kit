...
- **271 rules** across 12 security categories
- **86 scanner modules** including Python/TypeScript/Rust taint analysis
...
agent_audit_kit/
  ...
  rules/
    builtin.py         # 271 RuleDefinition entries (rule registry)
...
- **Rule registry**: `rules/builtin.py` defines all 271 rules as `RuleDefinition` dataclasses in a global `RULES` dict, populated by `_r()` helper.
