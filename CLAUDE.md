...
- **326 rules** across 14 security categories
- **94 scanner modules** including AST-based Python taint analysis plus regex dangerous-sink pattern scanners for TypeScript/JavaScript and Rust (pattern matching, not taint flow)
...
agent_audit_kit/
  ...
  rules/
    builtin.py         # 326 RuleDefinition entries (rule registry)
  scanners/            # 94 registered scanners (97 .py files on disk — the registry is authoritative)
...
- **Rule registry**: `rules/builtin.py` defines all 326 rules as `RuleDefinition` dataclasses in a global `RULES` dict, populated by `_r()` helper.
